...
--
-- Host: 127.0.0.1
-- Generation Time: 2016-04-22 17:42:08
-- 服务器版本： 5.6.17
-- PHP Version: 5.5.12
...
(1, 'admin', 0, '0000-00-00', '', 20, 20, 0, 1460863718, 0, 1461331732, 1),
(2, 'swc', 0, '0000-00-00', '', 10, 2, 0, 1460986400, 0, 1461037424, 1);
...
INSERT INTO `ot_reader` (`user_id`, `mobile`, `sex`, `college`, `major`, `name`) VALUES
(1, '15957190991', 1, '计算机学院', '计算机科学与技术', '沈伟臣'),
(2, '213', 0, '213', '213', '滕越');

-- --------------------------------------------------------

--
-- 替换视图以便查看 `ot_reader_view`
--
...
(1, 'admin', 'ab51f9d10187832ceb782655beae0eb3', '519589356@qq.com', '', 1460863718, 0, 1461331732, 0, 1460863718, 1),
(2, 'swc', '7c315f84bad9c7faf8f4120224ecf36d', 'last.fantasy@qq.com', '', 1460986386, 0, 1461037424, 0, 1460986386, 1);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ot_reader_view` AS select `ot_member`.`uid` AS `user_id`,`ot_member`.`nickname` AS `nickname`,`ot_reader`.`name` AS `name`,`ot_member`.`reg_time` AS `reg_time`,`ot_member`.`last_login_time` AS `last_login_time`,`ot_reader`.`mobile` AS `mobile`,`ot_reader`.`sex` AS `sex`,`ot_reader`.`college` AS `college`,`ot_reader`.`major` AS `major` from (`ot_member` left join `ot_reader` on((`ot_member`.`uid` = `ot_reader`.`user_id`)));

-- --------------------------------------------------------

--
-- 视图结构 `ot_recommend_view`
--
